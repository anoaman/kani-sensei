-- Kani Sensei platform — Phase 0: data spine (Turso / libSQL, SQLite dialect).
...
-- Apply via: turso db shell <db> < migrations/000_data_spine.sql

-- Cached WK subject catalog (kanji / vocabulary / radical).
create table if not exists wk_subjects (
    id              integer primary key,             -- WK subject id
    ...
    readings        text not null default '[]',      -- json array (as returned by WK)
    meanings        text not null default '[]',      -- json array
    raw             text,                            -- full WK payload for future needs
    synced_at       text
);
...
create table if not exists sync_runs (
    ...
    counts      text not null default '{}',           -- json {subjects, review_stats, assignments}
    error       text
);
